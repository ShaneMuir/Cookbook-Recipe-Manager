...
def search():
    ...
    results = mongo.db.recipe.find({'$text': {'$search': db_query}}).sort(
    ...
    if 'logged_in' in session:
        ...
        return render_template('search.html',
                               ...
                               db_query=db_query, current_user=current_user,
                               title="Search Results")
    else:
        return render_template('search.html',
                               results=results, pages=pages,
                               current_page=current_page, db_query=db_query,
                               title="Search Results")
...
def delete_recipe(recipe_id):
    ...
    if session:
        user = mongo.db.user.find_one({"name": session['username'].title()})
        the_recipe = mongo.db.recipe.find_one_or_404({
                                                '_id': ObjectId(recipe_id)})
